...
# Button, um eine neue Datenbank zu erstellen
new_data_button = tk.Button(root, text="neue Datenbank", command=new_database,width = 17, height=5)
new_data_button.grid(row = 0, column = 2)
# Button, um eine bestehende Datenbank einzulesen
read_data_button = tk.Button(root, text="Datenbank auswählen", command=conn_database,width = 17, height=5)
read_data_button.grid(row = 1, column = 2)
...
timetol_label = tk.Label(root, text = "Zeit-Toleranz")
timetol_label.grid(row=3,column=0,pady=8)
dist_label = tk.Label(root, text="Distanz 3 Messungen")
...
def noisefilter():
    ...
    try:
       ...
       # Create a connection to the SQLite database
        # conn = sqlite3.connect(read_data_button)          # Write the dataframe to a table in the SQLite database
        df_filtered.to_sql('eeltable', connection, if_exists='append',index = False)
    except:
        ...
        df_filtered.to_sql('eeltable', connection, if_exists='replace',index = False)
...
# def delduplicate():
#     """Lösche alle Duplikate innerhalb der Excel-Tabelle anhand der Subsets DATUM, Ort und Receiver"""
#     # Öffne Dialogfenster zur Öffnung der zu prüfenden Datei
#     root.fileinput1 = filedialog.askopenfilename(initialdir = "C:/)", title="Wähle eine Datei.", filetypes=(("xlsx files","*.xlsx"),("Alle Dateien","*.*")))
#     df1 = pd.read_excel(root.fileinput1).drop_duplicates(subset=['DATUM','Ort','Receiver']).reset_index(drop = True)
#     # Schreibe Datei neu ohne Duplikate
#     try:
#         with pd.ExcelWriter(root.fileinput1, mode="w", engine="openpyxl") as writer:
#             df1.to_excel(writer,sheet_name = 'Hydrophondaten', index=False)
#         messagebox.showinfo("Duplikate gelöscht","Duplikate wurden gelöscht.")
#     except:
#         messagebox.showerror("Error","Etwas ist schiefgelaufen. Vielleicht ist die Excel-Datei noch offen?!")
# # Duplikate-löschen-Button
# button1 = tk.Button(root, text="Duplikate löschen", command=delduplicate)
# root.grid_columnconfigure(3, minsize=270)
