...
# mother = pd.read_excel('Muttertabelle.xlsx',sheet_name = "Muttertabelle Farben Aale", header = 1, usecols=['ID Code', 'Punkte', 'Phenotype', 'Datum', 'Kontrast', 'KF', 'BF', 'OF'], dtype = {'ID Code':str})
# mother.to_pickle('motherpickel.pkl')
...
rangequery = "SELECT ID_HEX, Ort, Min(DATUM) AS min_date, MAX(DATUM) AS max_date FROM eeltable GROUP BY ID_HEX, Ort".strip()
...
date_ranges['Ort'] = date_ranges['Ort'].str.strip()
date_ranges['Ort'] = date_ranges['Ort'].astype(str)
date_ranges['min_date'] = pd.to_datetime(date_ranges['min_date'])
date_ranges['max_date'] = pd.to_datetime(date_ranges['max_date'])


def create_dots(x, y, x_offset, y_offset, radius, fill_color):
    """Fügt dem Canvas einen Punkt zu und nimmt Position, Offset, Radius und Farbe"""
    canvas.create_oval(x - radius + x_offset, y - radius + y_offset, x + radius + x_offset, y + radius + y_offset, fill=fill_color)


def animate_dots(df, delay):
    """ Animiert die Punkte, die auf der Map erscheinen"""
    global location_coordinates

    # Loop durch einen gegebenen dataframe
    for index, row in df.iterrows():
        # extrahiere den Ort aus der Reihe
        location = row.Ort
        # extrahiere Koordinaten für diesen Ort
        coordinates = location_coordinates[location]
        x, y = coordinates
        # Setze einen zufälligen Offset
        x_offset = random.randint(-7, 7)
        y_offset = random.randint(-7, 7)
        radius = 5
        fill_color = 'red'
        # Animiere die verschiedenen Punkte, indem create_dots gerufen wird
        canvas.after(delay*index, create_dots, x, y, x_offset, y_offset, radius, fill_color)
        canvas.update()

animate_dots(date_ranges,10)


def show_dataframe(loc, pheno):
    # Filter the dataframe based on loc and pheno
    filtered_df = eelcard_df[(eelcard_df['latest_loc'] == loc) & (eelcard_df['Phenotype'] == pheno)]
